...
int init()
{
	...
	listPoint.push_back(Point(0., 40., -50.));
	listPoint.push_back(Point(100., -100., -30.));
	listPoint.push_back(Point(0., 80., 80.));
	listPoint.push_back(Point(-50, -120., -30.));
	listPoint.push_back(Point(0., 100., -100.));

	//drawLine(line, listPoint);
	std::cout << "" << std::endl;
	//std::cout<< <<std::endl;

	std::vector<Point> subPoint = subdivisionPoint(listPoint, 8, 0.75);
	/*for (int i = 1; i < subPoint.size() - 1; i++) {
		std::cout << GetAnglesBetween2Points(subPoint[i - 1], subPoint[i]) << std::endl;
	}*/

	//drawLineWithVec(line, subPoint);
	buildPipe(line, subPoint);
